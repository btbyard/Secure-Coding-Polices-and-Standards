...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    //provide one string parameter
    //uses bind_param
    //sql->bind_param("s", NAME);
    //execute query
    /*
    bool inject_check(std::string sql) {
        regex str(".*(['A-Za-z0-9]+)\\s*=\\s*\\1.*");
        if (regex_match(sql, str)) {
            cout << "Possible SQL injection" << endl;
            return true;
        }
        else return false;
    }
    */
    ...
    // Used RegEx approach to fix SQL ijection
    ...
    if (std::regex_search(sql, re)) {
        ...
        std::cout << "POSSIBLE SQL INJECTION DETECTED !" << std::endl;
        return false;
    }
